...
class SafeLinksPolicyHandler:
    ...
    def check_policies(self) -> List[Dict[str, Any]]:
        """Check Safe Links policies against requirements using shared session manager"""
        all_results = []
        
        print("\n=== Starting Defender for Office 365 Safe Links Policy Check ===")
        
        if not self.requirements:
            print("No Safe Links requirements file provided")
            return [{
                'requirement_name': 'No Requirements',
                'found': False,
                'status': 'MISSING - No Safe Links requirements file provided',
                'policy_type': 'safelinks'
            }]
        
        print("Retrieving Safe Links policies using shared session manager")
        
        # Use shared session manager to get Safe Links policies
        all_policies = self.session_manager.get_all_defender_policies(['safelinks'])
        
        safelinks_policies = all_policies.get("safelinks", [])
        
        print(f"Retrieved {len(safelinks_policies)} Safe Links policies")
        
        # Check Safe Links policies against requirements
        print("\n--- Checking Safe Links Policies ---")
        safelinks_results = self._check_policy_requirements(safelinks_policies, self.requirements, "safelinks")
        all_results.extend(safelinks_results)
        
        return all_results

    def _check_policy_requirements(self, policies: List[Dict], requirements: Dict, policy_type: str) -> List[Dict[str, Any]]:
        """Check policies against requirements for Safe Links policies"""
        results = []
        ...
        for requirement in policy_requirements:
            ...
            for policy in enabled_policies:
                ...
                if isinstance(expected_value, bool):
                    ...
                    if expected_value == "list_present":
                        # Special case for DoNotRewriteUrls - check if it's a list (can be empty)
                        if current_value is not None:
                            if isinstance(current_value, list):
                                is_compliant = True  # List exists, can be empty or contain URLs
                            elif isinstance(current_value, str):
                                is_compliant = True  # String representation of list
                            else:
                                is_compliant = False
                        else:
                            is_compliant = False
    ...
    def get_requirements_summary(self) -> Dict[str, Any]:
        ...
        safelinks_count = len(self.requirements.get('safelinks_policies', []))
        
        return {
            'loaded': True,
            'requirements_count': safelinks_count,
            'safelinks_policies': safelinks_count
        }
